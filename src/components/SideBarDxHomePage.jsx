  ...
  fetch(url, {
    ...
    .then((res) => {
      if (res.ok) {
        ...
        throw new Error(`error in the first .then (GET): ${res.status} - ${res.statusText}`);
      }
    })
    .then((res) => {
      setData(res.items);
    ...
    .catch((e) => {
      ...
      console.log(`Error communicating with the server, please try again. Error: ${e}`);
    });
};
...
export default function SideBarDxHomePage() {
  ...
  useEffect(() => {
    getNews("https://api.rss2json.com/v1/api.json?rss_url=https://news.google.com/rss/headlines/section/topic/BUSINESS?hl=it&gl=IT&ceid=IT:it", setNews);
  }, []);
        ...
        <ul className="list-unstyled m-0 px-3 pb-2">
          {news.slice(0, 5).map((n) => (
            <li key={n.guid} className="py-2">
              <a href={n.link} target="_blank" className="text-decoration-none text-dark d-flex gap-2">
                <span
                  style={{
                    width: 6,
                    height: 6,
                ...
                <div className="overflow-hidden">
                  <div className="fw-semibold small w-100 text-truncate" style={{ lineHeight: 1.2 }}>
                    {n.title}
                  </div>
                  <div className="text-muted" style={{ fontSize: 12 }}>
                    {Math.round((new Date() - new Date(n.pubDate)) / (1000 * 60 * 60)) < 23
                      ? Math.round((new Date() - new Date(n.pubDate)) / (1000 * 60 * 60)) + " ore fa"
                      : Math.round((new Date() - new Date(n.pubDate)) / (1000 * 60 * 60 * 24)) === 1
                        ? Math.round((new Date() - new Date(n.pubDate)) / (1000 * 60 * 60 * 24)) + " giorno fa"
                        : Math.round((new Date() - new Date(n.pubDate)) / (1000 * 60 * 60 * 24)) + " giorni fa"}
                  </div>
                </div>
              </a>
            </li>
